...
async def test_full_loop_dod_passes_with_zero_crashes() -> None:
    result = await run_full_loop_dod_eval(runs_per_config=6, max_rounds=12)
    ...
    # With a 12-round budget, the wide and asymmetric ZOPAs agree; no-ZOPA never does.
...
async def test_full_loop_dod_reports_tool_usage() -> None:
    result = await run_full_loop_dod_eval(runs_per_config=2, max_rounds=12)
...
async def test_full_loop_dod_requires_20_runs() -> None:
    result = await run_full_loop_dod_eval(runs_per_config=1, max_rounds=12)
