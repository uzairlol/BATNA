...
async def test_reasoning_offer_consistent_when_prose_matches_offer() -> None:
    ...
    report = await ToMAuditor().audit_turn(
        role="buyer", principal=_buyer_principal(), call_log=ToolCallLog(),
        reasoning_text=text, terms=terms,
    )
...
async def test_provenance_consistent_when_figure_was_fetched() -> None:
    ...
    report = await ToMAuditor().audit_turn(
        role="buyer", principal=_buyer_principal(),
        call_log=_benchmark_log(latest=95.0), reasoning_text=text, terms=terms,
    )
...
async def test_audit_turn_rejects_unknown_role() -> None:
    with pytest.raises(ValueError):
        await ToMAuditor().audit_turn(
            role="arbiter", principal=_buyer_principal(), call_log=ToolCallLog(),
            reasoning_text="x", terms=_terms(),
        )